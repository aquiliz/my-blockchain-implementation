...
public class Miner {
    ...
    private void validateBlockTransactions(Block block) {
        ...
        log.info("All {} transactions were successfully validated for block id={}", block.getTransactions().size(),
                block.getId());
    }

    private void validateTransfer(Map<String, BigDecimal> balances, Transaction transaction, String fromAddress, String toAddress) {
        BigDecimal openingBalanceFromAddress = BlockChain.getInstance().getBalanceForAddress(fromAddress);
        BigDecimal openingBalanceToAddress = BlockChain.getInstance().getBalanceForAddress(toAddress);
        ...
        if (transaction.getAmount().compareTo(balances.get(fromAddress)) == 1) {
            throw new InsufficientFundsException("Transaction id='" + transaction.getId() + "' declined. Address '"
                    + fromAddress + "' does not possess the amount of " + transaction.getAmount());
        } else {
